...
#    self.physicalsList = list()
    ...
    def appendCylinderFields(self, radius, length, x, y, z, xaxis, yaxis, zaxis, vin, vout, bg):
        cylinderlist = [3, radius, length, x, y,
                        z, xaxis, yaxis, zaxis, vin, vout, bg]
    ...
    def appendFrustumFields(self, rin1, rout1, rin2, rout2, length, x, y, z, xaxis, yaxis, zaxis, vin, vout, bg):
        frustumlist = [5, rin1, rout1, rin2, rout2, length,
                       x, y, z, xaxis, yaxis, zaxis, vin, vout, bg]
...
#    physicalsStr = bytes(physicals, encoding = 'utf-8')
        ...
        if self.dim == 2:
            ...
            gmshPlugin.GenerateMesh2D(surfacestr, eletypestr, c_int(self.elementOrder), c_int(self.method),
                                      c_int(self.smoothIteration), c_double(
                                          self.sizeFactor),
    ...
    def startGenerationFluidMesh(self):
        ...
        strcom = strcom[:-1]
        objstr = bytes(strcom, encoding='utf-8')
        gmshPlugin.generateFluidMesh(objstr, eletypestr, fluidStr, c_int(
